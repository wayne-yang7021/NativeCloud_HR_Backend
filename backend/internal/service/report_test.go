...
func TestGetTodayAttendanceSummary(t *testing.T) {
	...
	summary, err := GetTodayAttendanceSummary(userID)
	if err != nil {
...
func TestGenerateAlertList(t *testing.T) {
	...
	got, err := GenerateAlertList("2023-01-01", "2023-01-02")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGetManagedDepartments(t *testing.T) {
	...
	t.Run("有管理部門", func(t *testing.T) {
		// STEP 1：mock 查詢 employee -> organization_id
		mock.ExpectQuery(`SELECT organization_id FROM "employee" WHERE employee_id = \$1`).
			WithArgs("userA").
			WillReturnRows(sqlmock.NewRows([]string{"organization_id"}).AddRow("D001"))

		// STEP 2：mock 查詢 organization -> department names
		mock.ExpectQuery(`SELECT "name" FROM "organization" WHERE organization_id LIKE \$1`).
			WithArgs("D001%").
			WillReturnRows(sqlmock.NewRows([]string{"name"}).
				AddRow("D001").
				AddRow("D002")) // 假設查到了兩個部門

		got := GetManagedDepartments("userA")
		want := []string{"D001", "D002"}

		if !reflect.DeepEqual(got, want) {
			t.Errorf("GetManagedDepartments() mismatch:\nGot:  %v\nWant: %v", got, want)
		}
	})

	t.Run("無管理部門", func(t *testing.T) {
		// STEP 1：mock 查詢 employee -> organization_id（userB 有 orgID）
		mock.ExpectQuery(`SELECT organization_id FROM "employee" WHERE employee_id = \$1`).
			WithArgs("userB").
			WillReturnRows(sqlmock.NewRows([]string{"organization_id"}).AddRow("D999"))

		// STEP 2：mock 查詢 organization -> department names（找不到）
		mock.ExpectQuery(`SELECT "name" FROM "organization" WHERE organization_id LIKE \$1`).
			WithArgs("D999%").
			WillReturnRows(sqlmock.NewRows([]string{"name"})) // 空結果

		got := GetManagedDepartments("userB")
		want := []string{}
		if !reflect.DeepEqual(got, want) {
			t.Errorf("GetManagedDepartments() mismatch:\nGot:  %v\nWant: %v", got, want)
		}
...
func TestGenerateAttendanceSummaryCSV(t *testing.T) {
	...
	got, err := GenerateAttendanceSummaryCSV("Engineering", "2023-01-01", "2023-01-02")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	want := [][]string{
		...
		{"2023-01-02", "E2", "Bob Chen", "", "", "", "", "Day Off"},
		{"2023-01-01", "E1", "Alice Wang", "", "", "", "", "Day Off"},
...
func TestGetAttendanceSummaryForDepartments(t *testing.T) {
	...
	got, err := GetAttendanceSummaryForDepartments("Engineering", "2023-01-01", "2023-01-02")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGenerateAttendanceSummaryPDF(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := GenerateAttendanceSummaryPDF(tt.args.dept, tt.args.start, tt.args.end)
...
func TestGetAttendanceWithEmployee(t *testing.T) {
	...
	got, err := GetAttendanceWithEmployee(userID, start, end)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFetchMonthlyTeamReport(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := FetchMonthlyTeamReport(tt.args.departmentID, tt.args.month)
			if (err != nil) != tt.wantErr {
...
func TestFetchCustomPeriodTeamReport(t *testing.T) {
	mockDB, mock := setupMockDB(t) // 你要實作這個 helper
	db.DB = mockDB
	...
	result, err := FetchCustomPeriodTeamReport(deptID, startDate, endDate)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
