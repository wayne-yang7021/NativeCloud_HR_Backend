...
func TestGetMyTodayRecords(t *testing.T) {
	...
	expected := `{"date":"2025-05-23","name":"John Doe","clock_in_time":"09:00","clock_out_time":"18:00","clock_in_gate":"A1","clock_out_gate":"B2","status":"Late"}`
...
func TestGetMyHistoryRecords(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodGet, "/history/"+userID, nil)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = req
	c.Params = gin.Params{gin.Param{Key: "userID", Value: userID}}

	GetMyHistoryRecords(c)
...
func TestGetMyPeriodRecords(t *testing.T) {
	...
	accessLogs := sqlmock.NewRows([]string{
		"access_id", "employee_id", "access_time", "direction", "gate_type", "gate_name", "access_result",
	}).AddRow("access-001", userID, start.Add(9*time.Hour), "IN", "TypeA", "Gate1", "Success").
		AddRow("access-002", userID, start.Add(18*time.Hour), "OUT", "TypeB", "Gate2", "Success")
	...
	reqURL := fmt.Sprintf("/period/%s/%s/%s", userID, start.Format("2006-01-02"), end.Format("2006-01-02"))
	...
	c.Params = gin.Params{
		...
		{Key: "startDate", Value: start.Format("2006-01-02")},
		{Key: "endDate", Value: end.Format("2006-01-02")},
	}

	GetMyPeriodRecords(c)
...
func TestGetInChargeDepartments(t *testing.T) {
	...
	// 🧪 模擬 user1 查詢 employee 表，取得 organization_id = "A"
	mock.ExpectQuery(`SELECT organization_id FROM "employee" WHERE employee_id = \$1`).
		WithArgs("user1").
		WillReturnRows(sqlmock.NewRows([]string{"organization_id"}).AddRow("A"))

	// 🧪 模擬 user1 查詢 organization 表 LIKE A%
	...
	// 🧪 模擬 user2 查詢 employee 表，無結果
	mock.ExpectQuery(`SELECT organization_id FROM "employee" WHERE employee_id = \$1`).
		WithArgs("user2").
		WillReturnRows(sqlmock.NewRows([]string{"organization_id"}).AddRow(""))

	// 🧪 模擬 user2 查詢 organization 表 LIKE '%'
	mock.ExpectQuery(`SELECT "name" FROM "organization" WHERE organization_id LIKE \$1`).
		WithArgs("%").
...
func TestGetThisMonthTeam(t *testing.T) {
	...
	if len(got) != 1 {
		t.Fatalf("expected 1 report maps (current & prev), got %d", len(got))
	}
...
func TestGetThisWeekTeam(t *testing.T) {
	...
	if len(got) != 1 {
		t.Fatalf("expected 1 week reports, got %d", len(got))
	}
